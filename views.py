...
class AboutView(MethodView):
    ...
    def post(self):
        ...
        if (
            "register" in request.form.to_dict().keys()
...
class RegistrationView(MethodView):
    ...
    def post(self):
        ...
        if "login" not in data:
            ...
            ):  # Вносим и сохраняем нового пользователя

                session["username"] = data["name"]

                return redirect(
                    "/your_passwords"
                )  # Перенаправляем на страницу с паролями пользователя
            else:
        ...
        if "create" in data:
            ...
            if "username" in session:
                ...
                db.save_password(
                    session["username"],
                    site_name,
                    Crypto.encrypt(password, user_password),
                )
    ...
    def post(self):
        ...
        if "create" in data:
            if "username" in session:
                ...
                db.save_password(
                    user_name, site_name, Crypto.encrypt(password, user_password)
                )
                return render_template("your_password_input.html")
            else:
                return redirect("/login")
        ...
        if "username" in session:
            ...
            if passwords:
                ...
                return render_template(
                    "your_passwords.html", message="У вас нет паролей"
                )
        else:
            return redirect("/login")
